...
export default function Header({
  ...
  return (
    <header className="sticky top-0 bg-gradient-to-br from-green-100 to-blue-100 z-20 w-full">
      ...
      <nav className="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8" aria-label="Global">
        <div className="flex lg:flex-1">
          <a href={`/${locale}`} className="-m-1.5 p-1.5" onClick={() => setShowLoadingModal(true)}>
            <Image className="h-8 w-auto" src="/appicona.svg" alt="Geometry Dash Scratch" width={32} height={32}/>
          </a>
          <a href={`/${locale}`} className="-m-1.5 ml-0.5 p-1.5" onClick={() => setShowLoadingModal(true)}>
            <Image
              className="h-8 w-auto"
              src="/appicontitle.svg"
              width={32}
              height={24}
              alt="Geometry Dash Scratch"/>
          </a>
        </div>
